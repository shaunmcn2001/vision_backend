...
# Patched: 10 m native reprojection, Drive export fix (no filePerBand in toDrive),
# streaming NDVI percentiles + classification to avoid OOM on 2GB instances.
...
DEFAULT_SMOOTH_RADIUS_M = 0   # smoothing disabled in this OOM-safe build
...
# Keep 10 m unless overridden by env.
DEFAULT_SCALE = int(os.getenv("ZONES_SCALE_M", "10"))

# Exports use a metric CRS; processing stays in native S2 meters.
DEFAULT_EXPORT_CRS = "EPSG:3857"
DEFAULT_CRS = DEFAULT_EXPORT_CRS

# Streaming threshold (pixels). Above this we strictly stream & skip smoothing.
ZONES_STREAM_TRIGGER_PX = int(os.getenv("ZONES_STREAM_TRIGGER_PX", "40000000"))  # ~40M px

# Palette used for rendering (metadata only)
...
def _download_image_to_path(image: ee.Image, geometry: ee.Geometry, target: Path) -> ImageExportResult:
...
# -----------------------
# Vectorization & zonal stats (stream-friendly)
# -----------------------
...
# -----------------------
# Local classification pipeline (reads mean_ndvi.tif)
# -----------------------
...
def _pixel_count(image: ee.Image, geometry: ee.Geometry, *, context: str, scale: int = DEFAULT_SCALE) -> int:
    try:
    ...
    try:
        values = list(info.values()); value = values[0] if values else 0
    ...
    *,
    ...
    if ndvi_collection is not None:
        try:
            ...
            mean_image = (
                ndvi_collection.mean().toFloat().setDefaultProjection(proj).reproject(proj, None, DEFAULT_SCALE)
            ).rename("NDVI_mean").clip(geometry)
            ndvi_stats["mean"] = mean_image.updateMask(mean_image.mask())
        except Exception:
            logger.exception("Failed to compute mean NDVI from collection")
    ...
    mean_export = _download_image_to_path(ndvi_stats["mean"].updateMask(stability_image), geometry, ndvi_path)
...
def build_zone_artifacts(
    ...
    *,
    ...
    if method_key != "ndvi_percentiles":
        raise ValueError("This build supports only 'ndvi_percentiles' method")
    try:
        gee.initialize()
    except Exception:
